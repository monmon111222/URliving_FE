...
const app = function (options) {
	...
	const vueApp = function () {
		// 地址綁訂&發票綁訂
		// const mergeApp = object.merge(address.app, invoice);
		const mergeApp = $.extend(true, {}, address.app, invoice);
		...
		fillInRecipient: function (data) {
			if (data !== null) {
				...
				if (window.isEmployee === "False") {
					_self.appShopping2.$refs.c_areatw.seletedCounty = window.recipientCity || "";
					_self.appShopping2.$refs.c_areatw.selectedZipcode = window.recipientDistrict || "";
					_self.appShopping2.$refs.c_areatw.selectedAddress = window.recipientAddress || "";
				}
			}
		},
	};
	...
	const bindEvent = function () {
		...
		$("#form-booking-info").validate({
			errorClass: "text-danger error h6 lh-15",
			errorElement: "div",
			rules: {
				name: {
					required: true,
