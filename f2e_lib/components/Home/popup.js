import Clipboard from "@global/vendors/clipboard.min";
require("jquery.cookie");

const popupAD = (function () {
	const $body = $("body");
	...
	if (typeof popupADConfig === "undefined" || $("#popupAD-container").length === 0) return;

	const dataPopupAD = popupADConfig.popupAD; // 只使用 popupAD 段
		...
		if (!dataPopupAD[i].note) {
			...
			if (startTime < serverTime && endTime > serverTime && !popupADCookie) {
				...
				setTimeout(() => {
					...
					const zIndex = dataPopupAD[i].zIndex ? parseInt($(popupWrapID).css("z-index")) + dataPopupAD[i].zIndex : "";
					...
					if (autoClose !== 0) {
						const popupTimeout = setTimeout(() => {
							closePopup(paramPopupAD);
							$.removeCookie(`${popupPrefix}${dataPopupID}`);
						}, autoClose);
